...
moverDisparo1 canonPos1 canonPos2 (Just (x, y)) hp1 hp2
    | y <= 0 = inputLoop canonPos1 canonPos2 Nothing Nothing hp1 (hp2 - 10) -- El disparo "desaparece" al llegar al suelo y resta HP al oponente
    | otherwise = do
        let newX = x + 1
            newY = max 0 (y - 1)
        mostrarCampo canonPos1 canonPos2 (Just (newX, newY)) Nothing
        inputLoop canonPos1 canonPos2 (Just (newX, newY)) Nothing hp1 hp2
...
moverDisparo2 canonPos1 canonPos2 (Just (x, y)) hp1 hp2
    | y <= 0 = inputLoop canonPos1 canonPos2 Nothing Nothing (hp1 - 10) hp2 -- El disparo "desaparece" al llegar al suelo y resta HP al oponente
    | otherwise = do
        let newX = x - 1
            newY = max 0 (y - 1)
        mostrarCampo canonPos1 canonPos2 Nothing (Just (newX, newY))
        inputLoop canonPos1 canonPos2 Nothing (Just (newX, newY)) hp1 hp2
